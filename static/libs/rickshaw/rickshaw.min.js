(function(root, factory) {
    ...
    Rickshaw.Graph = function(args) {
        ...
        this.validateSeries = function(series) {
            ...
            series.forEach(function(s) {
                ...
                if (s.data.length >= 3) {
                    if (
                        s.data[2].x < s.data[1].x ||
                        s.data[1].x < s.data[0].x ||
        ...
        this.discoverRange = function() {
            ...
            this.x = (this.xScale || d3.scale.linear())
                .copy()
                .domain(domain.x)
                .range([0, this.width]);
            this.y = (this.yScale || d3.scale.linear())
                .copy()
                .domain(domain.y)
                .range([this.height, 0]);
            this.x.magnitude = d3.scale
                .linear()
    ...
    Rickshaw.Graph.Axis.X = function(args) {
        ...
        this.render = function() {
            ...
            var bar_offset =
                (this.graph.renderer.name == 'bar' &&
                    Math.ceil((this.graph.width * 0.95) / this.graph.series[0].data.length / 2)) ||
                0;
            var transform;
    ...
    Rickshaw.Graph.Axis.Y.Scaled = Rickshaw.Class.create(Rickshaw.Graph.Axis.Y, {
        ...
        _drawAxis: function($super, scale) {
            ...
            var extentMap = d3.scale
                .linear()
                .domain([0, 1])
                .range(extents);
            var adjExtents = [extentMap(renderDomain[0]), extentMap(renderDomain[1])];
            var adjustment = d3.scale
                .linear()
                .domain(extents)
                .range(adjExtents);
            var adjustedScale = this.scale
                .copy()
        ...
        this.addAnchor = function(line) {
            ...
            anchor.onclick = function(e) {
                if (line.series.disabled) {
                    ...
                    if (
                        this.graph.series.filter(function(s) {
                            return !s.disabled;
                        }).length <= 1
                    )
                        return;
    ...
    Rickshaw.Graph.DragZoom = Rickshaw.Class.create({
        initialize: function(args) {
            ...
            var defaults = { opacity: 0.5, fill: 'steelblue', minimumTimeSelection: 60, callback: function() {} };
            this.graph = args.graph;
        ...
        registerMouseEvents: function() {
            ...
            var rectangle;
            var drag = { startDt: null, stopDt: null, startPX: null, stopPX: null };
    ...
    Rickshaw.Graph.HoverDetail = Rickshaw.Class.create({
        ...
        _addListeners: function() {
            ...
            this.mouseoutListener = function(e) {
                if (
                    e.relatedTarget &&
                    !(e.relatedTarget.compareDocumentPosition(this.graph.element) & Node.DOCUMENT_POSITION_CONTAINS)
                ) {
                    this.hide();
                }
            }.bind(this);
            ...
            $(function() {
                $(element).slider({
                    ...
                    slide: function(event, ui) {
                        ...
                        for (var i = 0; i < graphs.length; i++) {
                            self.processSlideChange({ event: event, ui: ui, graph: graphs[i] });
                        }
                    },
                });
            });
            graphs[0].onConfigure(
                function() {
    ...
    Rickshaw.Graph.RangeSlider.Preview = Rickshaw.Class.create({
        ...
        render: function() {
            ...
            var graphContainer = d3
                .select(this.element)
                .selectAll('div.rickshaw_range_slider_preview_container')
                .data(this.graphs);
            var translateCommand =
                'translate(' + this.config.frameHandleThickness + 'px, ' + this.config.frameTopThickness + 'px)';
            graphContainer
                .enter()
                .append('div')
            ...
            this.currentFrame[0] =
                currentWindow[0] === undefined ? 0 : Math.round(domainScale.invert(currentWindow[0]));
            if (this.currentFrame[0] < 0) this.currentFrame[0] = 0;
            this.currentFrame[1] =
                currentWindow[1] === undefined ? this.previewWidth : domainScale.invert(currentWindow[1]);
        ...
        _renderDimming: function() {
            ...
            path +=
                ' H ' +
                Math.min(
                    this.currentFrame[1] + this.config.frameHandleThickness * 2,
        ...
        _renderFrame: function() {
            ...
            path += ' z';
            path +=
                ' M ' + (this.currentFrame[0] + this.config.frameHandleThickness) + ' ' + this.config.frameTopThickness;
        ...
        _renderGrippers: function() {
            ...
            [0.4, 0.6].forEach(
                function(spacing) {
                    path +=
                        ' M ' +
                        Math.round(this.currentFrame[0] + this.config.frameHandleThickness * spacing) +
                        ' ' +
                        Math.round(this.config.height * 0.3);
                    path += ' V ' + Math.round(this.config.height * 0.7);
                    path +=
                        ' M ' +
                        Math.round(this.currentFrame[1] + this.config.frameHandleThickness * (1 + spacing)) +
                        ' ' +
                        Math.round(this.config.height * 0.3);
                    path += ' V ' + Math.round(this.config.height * 0.7);
                }.bind(this)
    ...
    Rickshaw.Graph.Renderer.Bar = Rickshaw.Class.create(Rickshaw.Graph.Renderer, {
        name: 'bar',
        defaults: function($super) {
            var defaults = Rickshaw.extend($super(), { gapSize: 0.05, unstack: false, opacity: 1 });
    ...
    Rickshaw.Series = Rickshaw.Class.create(Array, {
        initialize: function(data, palette, options) {
            ...
            this.timeBase =
                typeof options.timeBase === 'undefined' ? Math.floor(new Date().getTime() / 1e3) : options.timeBase;
